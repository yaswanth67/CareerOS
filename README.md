...
## Using OmniRoute as your AI backend (Claude Code, this app, and opencode)

[OmniRoute](https://omniroute.online/) is a local AI gateway on `http://localhost:20128` that fronts 200+ model providers behind one OpenAI-compatible endpoint, with automatic fallback and token compression. It is a drop-in replacement for the Claude Code connection this app expects — and the same endpoint also powers opencode.

### 1. Run the gateway

```bash
npm i -g omniroute   # or Docker / desktop app, see OmniRoute docs
# start it, then confirm the endpoint is up:
curl http://localhost:20128/v1/models
```

### 2. Point this app at it

The app already defaults to `http://localhost:20128` (see [How the AI connection works](#how-the-ai-connection-works)). In `.env`, set the token and model for your gateway:

```env
ANTHROPIC_BASE_URL="http://localhost:20128"
ANTHROPIC_AUTH_TOKEN="sk_omniroute"          # or the key from your gateway
ANTHROPIC_MODEL="auto/best-coding-fast"      # any id from /v1/models
```

### 3. Point opencode at it

Add the provider to `~/.config/opencode/opencode.json` (or your project's `opencode.json`):
...
```jsonc
{
  ...
  "provider": {
    "omniroute": {
      ...
      "models": {
        "auto/best-coding-fast": { "name": "auto/best-coding-fast" },
        "auto/best-coding":      { "name": "auto/best-coding" },
        "oc/claude-sonnet-4-6":  { "name": "Claude Sonnet 4.6" },
        "oc/claude-opus-4-6":    { "name": "Claude Opus 4.6" }
      }
    }
  }
}
...
Then restart opencode and pick a model: `/models` → **OmniRoute** → `auto/best-coding` (or any id you listed). Model ids must match your gateway's catalog exactly — run `curl http://localhost:20128/v1/models` to see what yours exposes.

Alternatives: `omniroute config opencode` merges this entry non-destructively, or install the helper `npm i -D @omniroute/opencode-provider` and call `buildOmniRouteOpenCodeConfig()`. The official plugin docs live at [npmjs.com/package/@omniroute/opencode-provider](https://www.npmjs.com/package/@omniroute/opencode-provider).
